...
public struct FlutterModelField {
    ...
    init(serializedData: [String: Any]) throws {
        ...
        guard let name = serializedData["name"] as? String
            ...
            throw ModelSchemaError.parse(className: "FlutterModelField", fieldName: "name", desiredType: "String")
        }
        self.name = name

        if let inputTypeMap = serializedData["type"] as? [String: Any]{
            self.type = try FlutterModelFieldType(serializedData: inputTypeMap)
        }
        else{
            throw ModelSchemaError.parse(className: "FlutterModelField", fieldName: "type", desiredType: "[String: Any]")
        }

        guard let isRequired = serializedData["isRequired"] as? Bool
        else {
            throw ModelSchemaError.parse(className: "FlutterModelField", fieldName: "isRequired", desiredType: "Bool")
        }
        self.isRequired = isRequired

        guard let isArray = serializedData["isArray"] as? Bool
        else {
            throw ModelSchemaError.parse(className: "FlutterModelField", fieldName: "isArray", desiredType: "Bool")
        }
    ...
    public func convertToNativeModelField() -> ModelField {
        return ModelField.init(
            name: name,
            type: type.convertToNativeModelField(),
